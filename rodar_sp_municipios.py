"""
Coleta estabelecimentos SUS de 4 municípios de SP e gera SP_MUNICIPIOS_SUS_SIM.xlsx.
"""
import datetime
import pandas as pd
...
OUTPUT   = Path("resultados_sp_municipios.xlsx")
SAIDA    = "SP_MUNICIPIOS_SUS_SIM.xlsx"
HEADLESS = True

MUNICIPIOS = [
    {"cod_ibge": "3548500", "municipio": "Santos",          "uf": "SP"},
    {"cod_ibge": "3518800", "municipio": "Guarulhos",       "uf": "SP"},
    {"cod_ibge": "3530706", "municipio": "Mogi das Cruzes", "uf": "SP"},
    {"cod_ibge": "3525300", "municipio": "Jau",             "uf": "SP"},
]
...
def gerar_excel(df: pd.DataFrame):
    ...
    nomes_municipios = ", ".join([m["municipio"] for m in MUNICIPIOS])
    ...
    ws["A1"] = f"ESTABELECIMENTOS DE SAUDE — SP ({nomes_municipios}) — ATENDE SUS: SIM"
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for mun in MUNICIPIOS:
            print(f"\n{'='*60}")
            print(f"Coletando: {mun['municipio']}/{mun['uf']}")
            print(f"{'='*60}")
            try:
                ...
                print(f"  -> {len(resultados)} estabelecimentos coletados")
                todos.extend(resultados)
            except Exception as e:
                print(f"  ERRO em {mun['municipio']}: {e}")

        browser.close()

    print(f"\n{'='*60}")
    print(f"Total geral coletado: {len(todos)} estabelecimentos")
